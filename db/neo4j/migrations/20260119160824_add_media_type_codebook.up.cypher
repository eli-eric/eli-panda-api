...
ON CREATE SET mt.uid = randomUUID(), mt.name = "J - Peer Reviewed Article"
MERGE (mt)-[:BELONGS_TO_FACILITY]->(f);

MATCH (f:Facility {code: "B"})
MERGE (mt:MediaType {code: "C"})
ON CREATE SET mt.uid = randomUUID(), mt.name = "C - Chapter in Book or Technical Paper"
MERGE (mt)-[:BELONGS_TO_FACILITY]->(f);

MATCH (f:Facility {code: "B"})
MERGE (mt:MediaType {code: "D"})
ON CREATE SET mt.uid = randomUUID(), mt.name = "D - Article in Proceedings"
MERGE (mt)-[:BELONGS_TO_FACILITY]->(f);
