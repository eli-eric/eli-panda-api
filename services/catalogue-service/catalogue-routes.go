...
func MapCatalogueRoutes(e *echo.Echo, h ICatalogueHandlers, jwtMiddleware echo.MiddlewareFunc) {
	...
	e.POST("/v1/catalogue/service/type", m.Authorization(h.CreateCatalogueServiceType(), shared.ROLE_CATALOGUE_EDIT), jwtMiddleware)
	//update catalogue service type
	e.PUT("/v1/catalogue/service/type/:uid", m.Authorization(h.UpdateCatalogueServiceType(), shared.ROLE_CATALOGUE_EDIT), jwtMiddleware)
	//delete catalogue service type
	e.DELETE("/v1/catalogue/service/type/:uid", m.Authorization(h.DeleteCatalogueServiceType(), shared.ROLE_CATALOGUE_EDIT), jwtMiddleware)
}
