...
func MapPublicationsRoutes(e *echo.Echo, h IPublicationsHandlers, jwtMiddleware echo.MiddlewareFunc) {
	...
	e.GET("/v1/publications/export/riv/validate", m.Authorization(h.ValidateRiv(), shared.ROLE_PUBLICATIONS_VIEW), jwtMiddleware)
	...
	e.PUT("/v1/researcher/:uid/researcher-ids", m.Authorization(h.RememberResearcherID(), shared.ROLE_PUBLICATIONS_EDIT), jwtMiddleware)
