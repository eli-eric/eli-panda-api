...
func GetSuppliersAutoCompleteQuery(searchString string, limit int) (result helpers.DatabaseQuery) {
	result.Query = `MATCH(s:Supplier) WHERE apoc.text.clean(s.name) STARTS WITH apoc.text.clean($searchString) RETURN {uid: s.uid, name: s.name + case when s.CIN is not null then " (" + s.CIN + ")" else "" end, code: s.code} as suppliers ORDER BY suppliers.name ASC LIMIT $limit`
