...
type IFilesService interface {
	...
	SetMiniImageUrlToNode(uid string, url string) (err error)
}
...
func (svc *FilesService) SetMiniImageUrlToNode(uid string, url string) (err error) {

	session, _ := helpers.NewNeo4jSession(*svc.neo4jDriver)

	query := SetMiniImageUrlToNodeQuery(uid, url)
	err = helpers.WriteNeo4jAndReturnNothing(session, query)
