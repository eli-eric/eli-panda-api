...
func GetZonesCodebookQuery(facilityCode string, searchString string) (result helpers.DatabaseQuery) {
	...
	UNION
	MATCH(f:Facility{code:$facilityCode}) WITH f
	MATCH(z:Zone)-[:HAS_SUBZONE]->(sz)-[:BELONGS_TO_FACILITY]->(f) where ($searchString = '' OR toLower(sz.code) contains $searchString OR toLower(sz.name) contains $searchString OR toLower(z.name) contains $searchString) return {uid:sz.uid, name: z.code+"-"+sz.code + " - " + sz.name + " ("+  z.name + ")"} as zone order by z.code, sz.code`
